...
## calculate or retrieve matrix with all intersections

if(file.exists("./input/intersecs_all.rds")) {
  
  intersecs_all <- readRDS("./input/intersecs_all.rds")
  
} else {
  
  intersecs_all <- st_intersects(kbas, sparse = F)
  
  saveRDS(intersecs_all, "./input/intersecs_all.rds")
